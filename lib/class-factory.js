    ...
    function makeMethod (methodName, type, methodId, retType, argTypes, env) {
      ...
      function replaceArtImplementation (fn) {
        ...
        if (fn !== null) {
          ...
          Memory.writeU32(request, kFullDeoptimization);
          try {
            console.log('>>>');
            withRunnableArtThread(vm, env, thread => {
              console.log('performing GoActive');
              api['art::Dbg::GoActive']();
              console.log('performing RequestDeoptimization');
              api['art::Dbg::RequestDeoptimization'](request);
              console.log('performing ManageDeoptimization');
              api['art::Dbg::ManageDeoptimization']();
              console.log('performed ManageDeoptimization');
            });
            console.log('<<<');
          } catch (e) {
            console.log('Oops! Crashed at ' + e.context.pc + ':\n\t' + Thread.backtrace(e.context).map(DebugSymbol.fromAddress).join('\n\t'));
            Process.enumerateModulesSync()
              .forEach(m => {
                console.log(m.name + ': ' + m.base.toString() + '=>' + m.base.add(m.size).toString());
              });
          }
